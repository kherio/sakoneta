    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:14px;margin-bottom:18px;">
      <?php foreach ($fotos as $f): ?>
        <div style="border:1.5px solid var(--borde);border-radius:8px;padding:8px;text-align:center;">
          ...
          <?php endif; ?>
          <a href="competicion_foto_borrar.php?id=<?= (int)$f['id'] ?>&competicion_id=<?= (int)$id ?>&csrf_token=<?= e(tokenCsrf()) ?>" class="borrar" style="font-size:12px;display:block;margin-top:4px;" onclick="return confirm('¿Borrar este archivo?');">Borrar</a>
        </div>
      <?php endforeach; ?>
    </div>
  <?php elseif ($id): ?>
